  ...
  async run (bot, msg, args) {
    ...
    if (args[0] === 'tools' || args[0] === 'Tools') {
      ...
      collectables.forEach(collectable => {
        embed.addField(`${collectable.emoji} ${collectable.display}`, `ID: \`${collectable.id}\` | Price: **:coin: ${utils.addCommas(collectable.price)}** | ${collectable.description.replace('[PRE]', utils.getPrefix(msg.guild.id))}`)
      })
      msg.channel.send(embed)
    }
  }
