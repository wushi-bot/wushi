...
class WithdrawCommand extends Command {
  ...
  async run (bot, msg, args) {
    ...
    if (eco.get(`${msg.guild.id}.${msg.author.id}.bank`) === 0) {
      return this.client.emit('customError', 'You don\'t have any coins.', msg)
    }
    let amount 
    if (args[0] === 'all') {
      amount = eco.get(`${msg.guild.id}.${msg.author.id}.bank`)
    } else if (args[0] === 'half') {
      amount = eco.get(`${msg.guild.id}.${msg.author.id}.bank`) / 2
    }
    if (!amount) {
      amount = eco.get(`${msg.guild.id}.${msg.author.id}.bank`)
      if (isNaN(amount)) {
      ...
      if (amount > eco.get(`${msg.guild.id}.${msg.author.id}.bank`)) {
        return this.client.emit('customError', 'The amount you inserted is more than you have!', msg)
      }
    }
    eco.subtract(`${msg.guild.id}.${msg.author.id}.bank`, amount)
    eco.add(`${msg.guild.id}.${msg.author.id}.balance`, amount)
