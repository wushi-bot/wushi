...
class BuyCommand extends Command {
  ...
  async run (bot, msg, args) {
    ...
    if (eco.get(`${msg.guild.id}.${msg.author.id}.balance`) < item.price) {
      return this.client.emit('customError', `${item.emoji} Insufficient coins! | You are :coin: **${utils.addCommas(item.price - eco.get(`${msg.guild.id}.${msg.author.id}.balance`))}** off.`, msg)
    }
    if (item.max) {
      var count = {}
      eco.get(`${msg.guild.id}.${msg.author.id}.items`).forEach(function (i) { count[i] = (count[i] || 0) + 1 })
